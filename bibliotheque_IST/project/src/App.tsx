...
export default function App() {
  ...
  // Redirect logic: if user tries to access a view they're not allowed
  useEffect(() => {
    if (loading) return;
  ...
  useEffect(() => {
    ...
    async function countOverdue() {
      ...
      const { count } = await supabase.from('borrowings').select('id', { count: 'exact', head: true }).is('return_date', null).lt('due_date', today);
