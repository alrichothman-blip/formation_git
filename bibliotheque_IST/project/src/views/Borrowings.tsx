...
export default function Borrowings({ searchQuery, addTrigger, showToast, isAdmin }: BorrowingsProps) {
  ...
  async function fetchData() {
    setLoading(true);
    const [bRes, bookRes, memRes] = await Promise.all([
      supabase.from('borrowings').select('*, books(id, title, author, cover_url, available_copies, categories(color)), members(id, name, prenom, email, phone, address, parcours, annee_etude)').order('created_at', { ascending: false }),
      supabase.from('books').select('id, title, author, available_copies').order('title'),
      supabase.from('members').select('*').order('name'),
    ]);
    if (bRes.data) {
      const today = new Date().toISOString().split('T')[0];
      const data = bRes.data.map((b: any) => ({
        ...b,
        status: b.return_date ? 'returned' : (b.due_date < today ? 'overdue' : 'active')
      }));
      setBorrowings(data);
    }
    if (bookRes.data) setBooks(bookRes.data as any);
  ...
  async function handleBorrow() {
    ...
    setSaving(true);
    const { error: bError } = await supabase.from('borrowings').insert({ ...form, status: 'active' });
    if (bError) { showToast('error', 'Erreur lors de l\'emprunt.'); setSaving(false); return; }
  ...
  async function handleReturn(borrowing: any) {
    setSaving(true);
    const today = new Date().toISOString().split('T')[0];
    const { error } = await supabase.from('borrowings').update({ return_date: today, status: 'returned', notes: returnNotes || borrowing.notes }).eq('id', borrowing.id);
    if (!error) {
    ...
    if (error) showToast('error', 'Erreur lors du retour.');
    else showToast('success', 'Livre retourné avec succès!');
    setSaving(false);
  ...
  async function handleDelete(id: string, bookId: string, isActive: boolean) {
    if (isActive) {
