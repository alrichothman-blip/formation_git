...
import { updateMemberProfile } from '../lib/memberStorage';
...
export default function Members({ searchQuery, showToast }: MembersProps) {
  ...
  const [borrowingCounts, setBorrowingCounts] = useState<Record<string, number>>({});

  useEffect(() => { fetchData(); }, []);

  async function fetchData() {
    setLoading(true);
    const [membersRes, borrowRes] = await Promise.all([
      supabase.from('members').select('*').order('name'),
      supabase.from('borrowings').select('member_id').is('return_date', null),
    ]);
    if (membersRes.data) setMembers(membersRes.data);
    if (borrowRes.data) {
      const counts: Record<string, number> = {};
      borrowRes.data.forEach((b: any) => { counts[b.member_id] = (counts[b.member_id] || 0) + 1; });
  ...
  async function handleSave() {
    ...
    const { error } = await updateMemberProfile(selectedMember!.id, payload);
    if (error) showToast('error', 'Erreur lors de la mise à jour.');
    else showToast('success', 'Étudiant mis à jour!');
    setSaving(false);
    setModalMode(null);
    fetchData();
  }
  ...
  async function handleDelete(id: string) {
    ...
    if (error) showToast('error', 'Impossible de supprimer (emprunts actifs?).');
    else { showToast('success', 'Étudiant supprimé.'); fetchData(); }
