...
export default function Stats() {
  ...
  async function fetchStats() {
    setLoading(true);
    const [booksRes, membersRes, borrowingsRes, categoriesRes] = await Promise.all([
      supabase.from('books').select('id, title, total_copies, available_copies, category_id, categories(name, color)'),
      supabase.from('members').select('id, status, created_at'),
      supabase.from('borrowings').select('id, book_id, member_id, borrow_date, return_date, due_date, status, books(title)'),
    ...
    // Books by category
    const catCounts: Record<string, { name: string; color: string; count: number; available: number }> = {};
    books.forEach((b: any) => {
      const catId = b.category_id || 'none';
      const catName = b.categories?.name || 'Sans catégorie';
      const catColor = b.categories?.color || '#94a3b8';
      if (!catCounts[catId]) catCounts[catId] = { name: catName, color: catColor, count: 0, available: 0 };
      catCounts[catId].count += b.total_copies;
      catCounts[catId].available += b.available_copies;
    });
    const booksByCategory = Object.values(catCounts).sort((a, b) => b.count - a.count);

    // Most borrowed books
    const bookBorrowCounts: Record<string, { title: string; count: number }> = {};
    borrowings.forEach((b: any) => {
      if (!bookBorrowCounts[b.book_id]) bookBorrowCounts[b.book_id] = { title: b.books?.title || 'Inconnu', count: 0 };
      bookBorrowCounts[b.book_id].count++;
    });
    const topBooks = Object.values(bookBorrowCounts).sort((a, b) => b.count - a.count).slice(0, 5);
    ...
    borrowings.forEach((b: any) => {
      const key = b.borrow_date.slice(0, 7);
      if (key in monthlyData) monthlyData[key]++;
    });
    const monthly = Object.entries(monthlyData).map(([month, count]) => {
    ...
    const overdueLoans = borrowings.filter((b: any) => !b.return_date && b.due_date < today).length;
    const returnRate = borrowings.length > 0 ? Math.round((borrowings.filter((b: any) => b.return_date).length / borrowings.length) * 100) : 0;

    setData({ booksByCategory, topBooks, monthly, totalBooks: books.length, activeMembers, totalMembers: members.length, overdueLoans, returnRate, totalBorrowings: borrowings.length });
    setLoading(false);
  }
