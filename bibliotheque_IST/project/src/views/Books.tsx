...
export default function Books({ searchQuery, addTrigger, showToast, isAdmin }: BooksProps) {
  ...
  async function fetchData() {
    ...
    const [booksRes, catsRes] = await Promise.all([
      supabase.from('books').select('*, categories(id, name, color, icon)').order('title'),
  ...
  function openEdit(book: BookType) {
    setForm({
      ...
      category_id: book.category_id || '', description: book.description,
  ...
  async function handleSave() {
    ...
    const payload = {
      ...
      available_copies: modalMode === 'add' ? form.total_copies : selectedBook!.available_copies,
      ...
      publisher: form.publisher, updated_at: new Date().toISOString(),
    };

    if (modalMode === 'add') {
      const { error } = await supabase.from('books').insert(payload);
      if (error) { showToast('error', 'Erreur lors de l\'ajout: ' + error.message); }
      else { showToast('success', 'Livre ajouté avec succès!'); }
    } else {
      const { error } = await supabase.from('books').update(payload).eq('id', selectedBook!.id);
      if (error) { showToast('error', 'Erreur lors de la mise à jour: ' + error.message); }
      else { showToast('success', 'Livre mis à jour avec succès!'); }
    }
  ...
  async function handleDelete(id: string) {
    ...
    if (error) showToast('error', 'Erreur lors de la suppression.');
    else { showToast('success', 'Livre supprimé.'); fetchData(); }
  ...
  const filtered = useMemo(() => {
    return books.filter(b => {
      ...
      const matchCat = !filterCategory || b.category_id === filterCategory;
